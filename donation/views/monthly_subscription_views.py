...
def getAMonthlySubscription(request, pk):
	try:
		...
		serializer = MonthlySubscriptionSerializer(monthly_subscription)
...
def searchMonthlySubscription(request):
	...
	if len(monthly_subscriptions) > 0:
		...
		return Response({'detail': f"There are no monthly_subscriptions matching your search"}, status=status.HTTP_400_BAD_REQUEST)
...
def  createMonthlySubscription(request):
	...
	members = filtered_data.get('members')
