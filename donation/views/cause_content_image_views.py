...
from donation.serializers import CauseContentImageSerializer, CauseContentImageListSerializer, \
    CauseContentImageMinimalSerializer
...
@permission_classes([IsAuthenticated])
# @has_permissions([PermissionEnum.ATTRIBUTE_LIST.name])
def getAllCauseContentImageByMenuId(request, cause_id):
...
def createCauseContentImage(request):
    ...
    for i in range(len(filtered_data) - 2):
        try:
            ...
            CauseContentImage.objects.create(cause=cause_obj, head=head, image=image)
        except KeyError:
            pass
