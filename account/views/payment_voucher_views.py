...
def getAllPaymentVoucher(request):
	distinct_payment_vouchers = PaymentVoucher.objects.filter(ledger__name__in=['Cash', 'Bank']).order_by('-invoice_no').distinct('invoice_no')
	...
	if len(payment_vouchers) > 0:
		for payment_voucher in payment_vouchers:
			...
			payment_voucher_serializer = PaymentVoucherListCustomSerializer(payment_voucher)
			for key, value in payment_voucher_serializer.data.items():
				payment_voucher_dict[key] = value
			related_payment_vouchers = PaymentVoucher.objects.filter(invoice_no=invoice_no).exclude(pk=payment_voucher.id)
			...
			if len(related_payment_vouchers) == 1:
				...
				payment_voucher_dict['related_ledgers'] = str(related_payment_voucher.ledger.name)
...
def getAllPaymentVoucherByInvoiceNo(request, invoice_no):
	...
	if len(payment_vouchers):
		...
		if payment_vouchers[0].sub_ledger:
			response_data['sub_ledger'] = {"id": payment_vouchers[0].sub_ledger.id, "name": payment_vouchers[0].sub_ledger.name}
		else:
			response_data['sub_ledger'] = None
		if payment_vouchers[0].branch:
			response_data['branch'] = {"id": payment_vouchers[0].branch.id, "name": payment_vouchers[0].branch.name}
		else:
...
def searchPaymentVoucher(request):
	...
	payment_vouchers = PaymentVoucher.objects.filter(ledger__name__in=['Cash', 'Bank'], invoice_no=invoice_no)
...
def createPaymentVoucher(request):
	...
	payment_voucher_serializer = PaymentVoucherListSerializer(payment_voucher_objs, many=True)
...
def updatePaymentVoucher(request):
	...
	for payment_voucher in items:
		...
		if payment_voucher_id:
			payment_voucher_obj = PaymentVoucher.objects.get(pk=payment_voucher_id)
			payment_voucher_serializer = PaymentVoucherSerializer(payment_voucher_obj, data=payment_voucher_dict)
			if payment_voucher_serializer.is_valid():
				print('validated_data: ', payment_voucher_serializer.validated_data)
				payment_voucher_serializer.save()
			account_log_serializer = AccountLogSerializer(account_log_objs[index], data=payment_voucher_dict)
			if account_log_serializer.is_valid():
				account_log_serializer.save()
			index += 1
		else:
			new_payment_voucher_serializer = PaymentVoucherSerializer(data=payment_voucher_dict)
			if new_payment_voucher_serializer.is_valid():
				print('validated_data else block: ', payment_voucher_serializer.validated_data)
				new_payment_voucher_serializer.save()
			new_account_log_serializer = AccountLogSerializer(data=payment_voucher_dict)
