...
def getAccountReportForPaymentVoucher(request):
	payment_vouchers = PaymentVoucherFilter(request.GET, queryset=PaymentVoucher.objects.exclude(debit_amount=0))
	...
	if len(payment_vouchers) > 0:
		for payment_voucher in payment_vouchers:
			...
			payment_voucher_serializer = PaymentVoucherListSerializer(payment_voucher)
			...
			if len(related_payment_vouchers) == 1:
				...
				payment_voucher_dict['related_ledger'] = str(related_payment_voucher.ledger.name)
				pass
...
def getAccountSummaryForPaymentVoucher(request):
	payment_vouchers = PaymentVoucherFilter(request.GET, queryset=PaymentVoucher.objects.all())
	payment_vouchers = payment_vouchers.qs
	total_elements = payment_vouchers.count()
...
def getAccountReportForReceiptVoucher(request):
	receipt_vouchers = ReceiptVoucherFilter(request.GET, queryset=ReceiptVoucher.objects.exclude(credit_amount=0))
	...
	if len(receipt_vouchers) > 0:
		for receipt_voucher in receipt_vouchers:
			...
			receipt_voucher_serializer = ReceiptVoucherListSerializer(receipt_voucher)
			...
			if len(related_receipt_vouchers) == 1:
				...
				receipt_voucher_dict['related_ledger'] = str(related_receipt_voucher.ledger.name)
				pass
...
def getAccountSummaryForReceiptVoucher(request):
	receipt_vouchers = ReceiptVoucherFilter(request.GET, queryset=ReceiptVoucher.objects.all())
	receipt_vouchers = receipt_vouchers.qs
	total_elements = receipt_vouchers.count()
...
def getAccountReportForPurchase(request):
	...
	if len(purchases) > 0:
		for purchase in purchases:
			...
			related_purchases = Sales.objects.filter(invoice_no=invoice_no).exclude(pk=purchase.id)
			print('related_purchases: ', related_purchases)
			if len(related_purchases) == 1:
				related_purchase = related_purchases[0]
				purchase_dict['related_ledger'] = str(related_purchase.ledger.name)
				pass
...
def getAccountReportForContra(request):
	...
	if len(contras) > 0:
		for contra in contras:
			...
			related_contras = Sales.objects.filter(invoice_no=invoice_no).exclude(pk=contra.id)
