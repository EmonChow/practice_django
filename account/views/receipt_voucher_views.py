...
def getAllReceiptVoucher(request):
	receipt_vouchers = ReceiptVoucher.objects.filter(ledger__name__in=['Cash', 'Bank']).order_by('-invoice_no').distinct('invoice_no')
	...
	if len(receipt_vouchers) > 0:
		for receipt_voucher in receipt_vouchers:
			...
			receipt_voucher_serializer = ReceiptVoucherListCustomSerializer(receipt_voucher)
			for key, value in receipt_voucher_serializer.data.items():
				receipt_voucher_dict[key] = value
			related_receipt_vouchers = ReceiptVoucher.objects.filter(invoice_no=invoice_no).exclude(pk=receipt_voucher.id)
			...
			if len(related_receipt_vouchers) == 1:
				...
				receipt_voucher_dict['related_ledgers'] = str(related_receipt_voucher.ledger.name)
...
def getAllReceiptVoucherByInvoiceNo(request, invoice_no):
	...
	if len(receipt_vouchers) > 0:
		...
		if receipt_vouchers[0].sub_ledger:
			response_data['sub_ledger'] = {'id': receipt_vouchers[0].sub_ledger.id, 'name': receipt_vouchers[0].sub_ledger.name}
		else:
			response_data['sub_ledger'] = None
		if receipt_vouchers[0].branch:
			response_data['branch'] = {'id': receipt_vouchers[0].branch.id, 'name': receipt_vouchers[0].branch.name}
		else:	
...
def searchReceiptVoucher(request):
	...
	receipt_vouchers = ReceiptVoucher.objects.filter(ledger__name__in=['Cash', 'Bank'], invoice_no=invoice_no)
...
def createReceiptVoucher(request):
	...
	receipt_voucher_serializer = ReceiptVoucherListSerializer(receipt_voucher_objs, many=True)
...
def updateReceiptVoucher(request):
	...
	for receipt_voucher in items:
		...
		if receipt_voucher_id is not None:
			receipt_voucher_obj = ReceiptVoucher.objects.get(pk=receipt_voucher_id)
			receipt_voucher_serializer = ReceiptVoucherSerializer(receipt_voucher_obj, data=receipt_voucher_dict)
			if receipt_voucher_serializer.is_valid():
				receipt_voucher_serializer.save()
			account_log_serializer = AccountLogSerializer(account_log_objs[index], data=receipt_voucher_dict)
			if account_log_serializer.is_valid():
				account_log_serializer.save()
			index += 1
		else:
			new_receipt_voucher_serializer = ReceiptVoucherSerializer(data=receipt_voucher_dict)
			if new_receipt_voucher_serializer.is_valid():
				new_receipt_voucher_serializer.save()
			new_account_log_serializer = AccountLogSerializer(data=receipt_voucher_dict)
...
def createReceiptVoucherPREV(request):
	...
	for receipt_voucher in data:
		...
		receipt_datetime =  str(receipt_date) + 'T' + str(datetime.now().time()) + 'Z'
		print('receipt_datetime: ', receipt_datetime)
