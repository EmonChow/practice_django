...
from account.serializers import AccountLogSerializer, ContraListCustomSerializer, ContraSerializer, \
    ContraListSerializer, AccountLogListSerializer
...
def getAllContraByInvoiceNo(request, invoice_no):
    ...
    if len(contras):
        ...
        if contras[0].branch:
            response_data['branch'] = {"id": contras[0].branch.id, "name": contras[0].branch.name}
        else:
...
def searchContra(request):
    contras = ContraFilter(request.GET, queryset=Contra.objects.all().order_by('invoice_no').distinct('invoice_no'))
    contras = contras.qs
...
def createContra(request):
    ...
    contra_voucher_serializer = ContraListSerializer(contra_voucher_objs, many=True)
...
def updateContra(request):
    ...
    for contra in items:
        ...
        if contra_id:
            ...
            account_log_serializer = AccountLogSerializer(account_log_objs[index], data=contra_dict)
