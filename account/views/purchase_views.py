...
def getAllPurchase(request):
	purchases = Purchase.objects.filter(ledger__name='Company Purchase').order_by('-invoice_no').distinct('invoice_no')
	...
	if len(purchases) > 0:
		for purchase in purchases:
			...
			related_purchases = Purchase.objects.filter(invoice_no=invoice_no).exclude(pk=purchase.id)
			print('related_purchases: ', related_purchases)
			if len(related_purchases) == 1:
				related_purchase = related_purchases[0]
				purchase_dict['related_ledgers'] = str(related_purchase.ledger.name)
...
def getAllPurchaseByInvoiceNo(request, invoice_no):
	...
	if len(purchases) > 0:
		...
		if purchases[0].sub_ledger:
			response_data['sub_ledger'] = {'id': purchases[0].sub_ledger.id, 'name': purchases[0].sub_ledger.name}
		else:
			response_data['sub_ledger'] = None
		if purchases[0].branch:
			response_data['branch'] = {'id': purchases[0].branch.id, 'name': purchases[0].branch.name}
		else:
...
def searchPurchase(request):
	...
	purchases = Purchase.objects.filter(ledger__name='Company Purchase', invoice_no=invoice_no)
...
def updatePurchase(request):
	...
	for purchase in items:
		...
		if purchase_id:
			...
			purchase_serializer = PurchaseSerializer(purchase_obj, data=purchase_dict)
			if purchase_serializer.is_valid():
				purchase_serializer.save()
			account_log_serializer = AccountLogSerializer(account_log_objs[index], data=purchase_dict)
			...
			new_account_log_serializer = AccountLogSerializer(data=purchase_dict)
